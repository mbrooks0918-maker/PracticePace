...
function ClockFace({ secs, warn, size = 'lg' }) {
  const fs = size === 'lg' ? 'clamp(2.8rem,8vw,5rem)' : 'clamp(1.8rem,5vw,3rem)'
  return (
    <span className="font-mono font-black" style={{ fontSize: fs, color: warn ? '#ef4444' : '#fff', fontVariantNumeric: 'tabular-nums' }}>
      {fmtClock(secs)}
    </span>
  )
}

function SmallClockFace({ secs, warn }) {
  return (
    <span className="font-mono font-black" style={{ fontSize: 'clamp(2rem,6vw,3.5rem)', color: warn ? '#ef4444' : '#fff', fontVariantNumeric: 'tabular-nums' }}>
      {pad(secs)}
    </span>
  )
}
...
function FootballScoreboard({ orgColor }) {
  ...
  return (
    <div className="flex-1 flex flex-col gap-3 p-4 overflow-hidden min-h-0">
      ...
      {/* Row 1: Game clock + quarter + controls */}
      ...
      {/* Row 2: Teams — takes all remaining height */}
  ...
  return (
    <div className="flex-1 flex flex-col gap-3 p-4 overflow-hidden min-h-0">
        ...
        {[['home', home, setHome], ['away', away, setAway]].map(([side, team, setTeam]) => (
          <div key={side} className="flex-1 flex flex-col items-center justify-around gap-3 rounded-2xl p-4"
            ...
            <input value={team.name} onChange={e => setTeam(t => ({ ...t, name: e.target.value }))}
              ...
              style={{ backgroundColor:'transparent', border:'1px solid #2a0000', color:'#9a8080' }} maxLength={20} />
            {/* Possession dot */}
            <button onClick={() => setPossession(p => p === side ? null : side)}
              title="Possession"
      ...
      <div className="shrink-0 flex items-center gap-4 flex-wrap rounded-2xl px-6 py-4"
        ...
        <SmallClockFace secs={shotSecs} warn={shotSecs <= 5} />
